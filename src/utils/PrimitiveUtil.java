...
public class PrimitiveUtil {
	/**
	 * check bits parity for an array of 64-bit positive long numbers in a naive
	 * way.
	 * 
	 * @param nums
		...
		for (long num : nums) {
			if (num < 0)
				throw new IllegalArgumentException(
						"Please ensure that the array doesn't contain negative long numbers!");
	...
	/**
	 * check bits parity for an array of 64-bit positive long numbers in a more
	 * effective way.
	 * 
	 * @param nums
	 * @return true if parity is 1, otherwise return false.
	...
	public static boolean getBitsParityBetter(long[] nums) {
		...
		for (long num : nums) {
			...
			while (num > 0) {
				result ^= 1;
				num &= (num - 1); // num &= (num - 1) will drop the lowest set,
									// while num &= ~(num - 1) will retain the
									// lowest set only.
			}
		}
	 ...
	 * 
	 * @param b1
	 *            b1 should be between 2 and 16
	 * @param b2
	 *            b2 should be between 2 and 16
	 * @param num
	 *            the number to be converted, it should be an integer
	 * @return the number converted from base b1 to base b2, time complexity is
	 *         n + logb2(b1^n) = n(1+logb2(b1))
	 */
	public static String convertBase(int b1, int b2, String num) {
		if (b1 < 2 || b1 > 16 || b2 < 2 || b2 > 16 || num.length() == 0)
			throw new IllegalArgumentException(
					"b1 or b2 should be between 2 and 16!");
		...
		return positive ? result.reverse().toString() : "-"
